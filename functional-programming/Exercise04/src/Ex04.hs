...
ins key value (Leaf k d)
  | key < k = Branch (Leaf key value) Empty k d
  | key > k = Branch Empty (Leaf key value) k d
  | key == k = Leaf key value

ins key value (Branch left right k d)
  | key < k = Branch (ins key value left) right k d
  | key > k = Branch left (ins key value right) k d
  | key == k = Branch left right k value
...
lkp Empty _ = fail "can't lkp Empty BinTree"
...
getLASs length listsum sumofsquares (x:xs) =
  getLASs (length+1) (listsum+x) (sumofsquares+(x*x)) xs
